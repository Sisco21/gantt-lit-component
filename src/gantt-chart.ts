...
export class GanttChart extends LitElement {
  ...
  static styles = css`
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .search-control input { width: min(210px, 23vw); min-height: 30px; border: 1px solid var(--gantt-control-border); border-radius: 6px; background: var(--gantt-input-background); color: inherit; font: inherit; font-size: 12px; padding: 4px 7px; }
    ...
    .week { padding: 4px 4px 0; border-right: 2px solid var(--gantt-border); background: color-mix(in srgb, var(--gantt-header) 76%, var(--gantt-border)); font-size: 10px; font-weight: 700; }
    ...
    .day.week-start { border-left: 2px solid color-mix(in srgb, var(--gantt-muted) 42%, transparent); }
    ...
    .task-row.search-match, .timeline-row.search-match { background-color: #fffbed; }
    .task-row.search-current { box-shadow: inset 3px 0 0 #d99714; }
    .task-cells { height: 100%; align-items: center; }
    .task-cell { height: 100%; padding: 12px 5px 0; color: inherit; font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }
    .task-cell.name { position: relative; padding-top: 11px; }
    .task-cell.number { text-align: right; }
    .task-cell.parent { font-weight: 700; }
    .task-row .toggle { position: absolute; left: 2px; top: 10px; z-index: 2; flex: 0 0 19px; width: 19px; height: 22px; padding: 0; border: 0; background: transparent; color: var(--gantt-muted); font-size: 11px; }
    ...
    .task-bar { position: absolute; top: 9px; z-index: 4; height: 24px; border-radius: 5px; background: var(--bar-color); box-shadow: inset 0 -2px rgb(0 0 0 / 10%); color: #fff; cursor: grab; font-size: 11px; line-height: 24px; overflow: hidden; padding: 0 7px; text-overflow: ellipsis; white-space: nowrap; }
    ...
    .task-span { position: absolute; inset: 0; border: 1px dashed var(--bar-color); border-radius: 5px; background: color-mix(in srgb, var(--bar-color) 18%, transparent); }
    .task-segment { position: absolute; top: 0; height: 24px; overflow: hidden; border-radius: 4px; background: var(--bar-color); box-shadow: inset 0 -2px rgb(0 0 0 / 10%); padding: 0 7px; white-space: nowrap; }
    .summary-bar { position: absolute; top: 10px; z-index: 4; height: 15px; border-top: 2px solid var(--gantt-summary); color: var(--gantt-summary); cursor: grab; overflow: visible; }
    .summary-cap { position: absolute; top: -2px; width: 2px; height: 9px; background: var(--gantt-summary); }
    ...
    .dependency-line.horizontal { height: 0; border-top: 2px dashed var(--dependency-color, #3b82c4); }
    .dependency-line.vertical { width: 0; border-left: 2px dashed var(--dependency-color, #3b82c4); }
    ...
    .resource-day.week-start { border-left: 2px solid color-mix(in srgb, var(--gantt-muted) 42%, transparent); }
    ...
    .resource-non-working-day-band { position: absolute; top: 0; bottom: 0; z-index: 3; border-left: 1px solid rgb(148 163 184 / 20%); background: var(--gantt-non-working-day); pointer-events: none; }
    ...
    .resource-day-input:disabled { background: var(--gantt-header); }
          ...
          <div class="gantt-viewport">
            ...
            <div class="gantt-layout">
              ...
              <div class="timeline-pane">
                <div class="timeline-scroll" @scroll=${this.syncTimelineHeaderScroll}>
  ...
  addChildTask(parentId: string, newTask: Partial<GanttTask> = {}): GanttTask {
    ...
    const task: GanttTask = {
      ...
      type: newTask.type || (parentId ? 'task' : 'parent'),
    };
    ...
    return html`
      <section class="resources-panel" aria-label="${this.t('resources')} — ${task.name}" style="--resource-columns-width:${resourceColumnsWidth}px; --resource-columns-template:${resourceColumnsTemplate}">
        ...
        <div class="resources-scroll" @contextmenu=${(event: MouseEvent) => this.openResourceContextMenu(task.id, event)}>
          ...
          ${this.renderResourceNonWorkingDayBands(visibleDates, windowOffset, dayWidth)}
          ${resources.map(resource => html`<div class="resource-day-row"><div class="resource-day-window" style="left:${windowOffset}px">${visibleDates.map(date => {
            const dateKey = formatDate(date);
            const active = dateKey >= task.start && dateKey <= task.end;
            const value = resource.quantityByDate?.[dateKey];
            return html`<input class="resource-day-input" type="number" min="0" step="1" ?disabled=${!active} value=${value === undefined ? '' : value} aria-label="${resource.name} ${dateKey}" @change=${(event: Event) => this.updateResourceQuantity(task.id, resource.id, dateKey, (event.target as HTMLInputElement).value)} />`;
          })}</div></div>`)}
              </div>
            </div>
          </div>
  ...
  private updateResourceColumn(task: GanttTask, resource: GanttResource, column: GanttResourceColumn, value: string): void {
    ...
    if (['name', 'type', 'unitCost', 'quantity'].includes(column.key)) {
  ...
  private renderTimelineRow(task: GanttTask, index: number, start: Date, dayWidth: number, searchMatches: Set<string>) {
    ...
    const color = task.color || this.getColors()[task.type] || (task.type === 'parent' ? 'var(--gantt-parent)' : task.type === 'milestone' ? 'var(--gantt-milestone)' : 'var(--gantt-task)');
    const selected = task.id === this.selectedTaskId;
    return html`
  ...
  private renderTaskBar(task: GanttTask, left: number, width: number, color: string, selected: boolean, timelineStart: Date, dayWidth: number) {
    ...
    if (summary) {
      const label = this.options.summaryTemplate?.(task) ?? task.name;
      return html`
        <div class="summary-bar ${selected ? 'selected' : ''}" style="left:${left}px; width:${Math.max(24, width)}px;"
          ...
          <span class="summary-cap start"></span><span class="summary-cap end"></span><span class="summary-label">${label}</span>
        </div>
      `;
    }
    ...
    if (segments) {
      return html`
        <div class="task-work ${selected ? 'selected' : ''}" style="left:${left}px; width:${width}px; --bar-color:${color};"
          ...
          ${segments.map((segment, index) => html`<div class="task-segment" style="left:${segment.left - left}px; width:${segment.width}px">${index === 0 && segment.width > 90 ? html`<span class="bar-label">${task.name}</span>` : nothing}</div>`)}
    ...
    return html`
      <div class="task-bar ${milestone ? 'milestone' : ''} ${selected ? 'selected' : ''}"
        ...
        ${milestone ? nothing : html`
          ...
          <span class="bar-label">${width > 90 ? task.name : ''}</span>
  ...
  private renderTimelineHeader(start: Date, totalDays: number, dayWidth: number) {
    ...
    return html`
      <div class="timeline-header ${this.options.showWeekNumbers ? 'with-weeks' : ''}">
        ...
        <div class="days">${dates.map(date => html`<div class="day ${this.isNonWorkingDay(date) ? 'weekend' : ''} ${this.isWeekStart(date) ? 'week-start' : ''}" style="width:${dayWidth}px" title=${this.formatDayTitle(date)}>${date.getUTCDate()}</div>`)}</div>
      </div>
    `;
  }
  ...
  private renderNonWorkingDayBands(start: Date, totalDays: number, dayWidth: number) {
    ...
    return Array.from({ length: totalDays }, (_, index) => {
      ...
      return this.isNonWorkingDay(date)
        ? html`<div class="non-working-day-band" style="left:${index * dayWidth}px; width:${dayWidth}px"></div>`
        : nothing;
    });
  }

  private renderResourceNonWorkingDayBands(dates: Date[], offset: number, dayWidth: number) {
    if (!this.getNonWorkingDays().length) return nothing;
    return dates.map((date, index) => this.isNonWorkingDay(date)
      ? html`<div class="resource-non-working-day-band" style="left:${offset + index * dayWidth}px; width:${dayWidth}px"></div>`
      : nothing);
  }
  ...
  private renderDependencies(visibleTasks: GanttTask[], taskById: Map<string, GanttTask>, start: Date, dayWidth: number) {
    ...
    const paths = this.dependencies.flatMap(dependency => {
      ...
      const goesDown = toRow >= fromRow;
      // Les ancrages se placent dans les marges des lignes, pas au milieu des barres.
      const y1 = fromRow * ROW_HEIGHT + (goesDown ? 35 : 7);
      const y2 = toRow * ROW_HEIGHT + (goesDown ? 7 : 35);
      const targetX = Math.max(0, toX);
      const hasClearGap = targetX > fromX + 4;
      // Pour deux tâches contiguës, un crochet court à droite évite une verticale
      // ambiguë qui semblerait appartenir à la barre elle-même.
      const elbow = hasClearGap
        ? fromX + Math.min(12, Math.max(3, (targetX - fromX) / 2))
        : fromX + 8;
      const verticalTop = Math.min(y1, y2);
      const horizontalStart = Math.min(elbow, targetX);
      const horizontalEndWidth = Math.abs(targetX - elbow);
      return [html`
        <div class="dependency-connector">
          <span class="dependency-line horizontal" style="left:${Math.min(fromX, elbow)}px; top:${y1}px; width:${Math.max(1, Math.abs(elbow - fromX))}px"></span>
          <span class="dependency-line vertical" style="left:${elbow}px; top:${verticalTop}px; height:${Math.max(1, Math.abs(y2 - y1))}px"></span>
          <span class="dependency-line horizontal" style="left:${horizontalStart}px; top:${y2}px; width:${Math.max(1, horizontalEndWidth)}px"></span>
  ...
  private applyData(data: GanttData, reason: GanttChangeReason, notify: boolean): void {
    ...
    this.tasks = buildTaskTree(scheduledTasks);
  ...
  private replaceFlatTasks(tasks: GanttTask[], reason: GanttChangeReason, taskId?: string): void {
    this.tasks = buildTaskTree(tasks);
    this.requestUpdate();
  ...
  private applyScheduledDates(
    ...
    const redistributeResourceDates = (task: GanttTask): void => {
      ...
      task.resources = task.resources.map(resource => {
        ...
        return { ...resource, quantity: total, quantityByDate: this.createDistributedQuantities(task.start, task.end, total), totalQuantity: undefined, cost: undefined };
      });
    };
  ...
  private createDistributedQuantities(start: string, end: string, requestedTotal: number): Record<string, number> | undefined {
    ...
    for (let index = 0; index < days; index += 1) {
      ...
      if (value > 0) quantityByDate[this.addDays(start, index)] = value;
      allocated += value;
    }
  ...
  private updateResource(taskId: string, resourceId: string, field: keyof GanttResource, value: unknown): void {
    const next = this.getFlatTasks().map(task => {
      ...
      const resources = (task.resources || []).map(resource => resource.id === resourceId ? { ...resource, [field]: value, cost: field === 'unitCost' || field === 'quantity' ? Number(field === 'unitCost' ? value : resource.unitCost) * Number(field === 'quantity' ? value : resource.quantity) : resource.cost } : resource);
  ...
  private distributeResourceTotal(taskId: string, resourceId: string, rawValue: string): void {
    ...
    const next = this.getFlatTasks().map(task => {
      ...
      const resources = (task.resources || []).map(resource => resource.id === resourceId
        ? { ...resource, quantity: total, quantityByDate: this.createDistributedQuantities(task.start, task.end, total), totalQuantity: undefined, cost: undefined }
        : resource);
      return { ...task, resources };
    });
    this.replaceFlatTasks(next, 'task-updated', taskId);
